...
2. **Initialize a workspace in demo mode**
   ...
   The `--demo` flag creates a standard Stoneforge workspace and automatically configures all default agents (director, workers, steward) to use the free `opencode/minimax-m2.5-free` provider.
...
### Via CLI (at initialization)

```bash
# Create a new workspace with demo mode
sf init --demo
```
