...
Sync tasks and documents with external tools (GitHub, Linear, Notion, local folders). All commands live under `sf external-sync`.
...
# Set default project for a provider
sf external-sync config set-project github org/repo
sf external-sync config set-project linear TEAM-KEY
sf external-sync config set-project notion <database-id>
sf external-sync config set-project folder ./docs

# Enable auto-linking for new elements
sf external-sync config set-auto-link github              # Tasks
sf external-sync config set-auto-link notion --type document  # Documents
...
Before syncing, elements must be linked to their external counterparts.

```bash
# Link a single element by URL or external ID
sf external-sync link <elementId> <url-or-id> [-p <provider>] [-t task|document]

# Bulk-link all unlinked elements
sf external-sync link-all -p <provider> [--project <project>] [--status <status>] [--dry-run] [--force] [--batch-size <n>]

# Remove a link
sf external-sync unlink <elementId>

# Bulk-remove all links
sf external-sync unlink-all [-p <provider>] [-t task|document|all] [--dry-run]
```
...
sf external-sync push [elementId...]           # Specific elements
sf external-sync push --all [-t task|document]  # All linked elements
sf external-sync push --all --force             # Skip hash comparison

# Pull changes from external systems
sf external-sync pull [-p <provider>] [-t task|document|all]
sf external-sync pull --discover               # Also import unlinked external items

# Bidirectional sync (push then pull)
sf external-sync sync [--dry-run] [-t task|document|all]
```

| Flag | Description |
| --- | --- |
| `-t, --type` | Filter by element type: `task`, `document`, or `all` |
| `-p, --provider` | Target a specific provider |
| `--discover` | Import unlinked external items as new elements |
| `--force` | Push regardless of hash comparison |
| `--dry-run` | Preview changes without applying them |

### Status and conflicts

```bash
# Show sync overview (linked counts, conflicts, cursors)
sf external-sync status

# Resolve a conflict
sf external-sync resolve <elementId> --keep local
sf external-sync resolve <elementId> --keep remote
```
